...
optimizer = LBFGS(model.parameters(), lr=0.5, history_size=200, line_search='Armijo', debug=True)
...
for n_iter in range(max_iter):
    ...
    options = {'closure': closure, 'current_loss': obj, 'interpolate': False}
    ...
    if n_iter * batch_size - ((n_iter * batch_size) // 50000) * 50000 < batch_size:
        ...
        # print data
        print('Iter: %d lr: %f Training loss: %f Test loss: %f Test acc: %.2f%%' % (
        n_iter + 1, lr, train_loss, test_loss, test_acc))
        end = time.time()
